...
export default function ProjectDetail() {
	...
	const d = (children, type) => <div className={`${styles[`m${type[1]}`]} ${styles[type[0]]}`}>{children}</div>;

	const p = (index) => <p className={styles[index[0]]}>{data.content[index]}</p>;

	const i = (index) => (
		<Image
			src={data.content[index].href}
			alt='img'
			width={data.content[index].width}
			height={data.content[index].height}
			loading='eager'
		/>
	);

	const l = (index) => (
		<div className={styles.lRoot}>
	...
	const g = (index) => {
		...
		return (
			<div className={styles.m5}>
				<a href={`https://github.com${data.content[index].href}`}>
